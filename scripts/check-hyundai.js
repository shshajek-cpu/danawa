...
const hyundaiCars = cars.cars.filter(c => c.brandId === 'hyundai');
hyundaiCars.forEach(c => {
  const d = details[c.id];
  if (!d) { console.log('[현대] ' + c.name + ' (ID:' + c.id + ') - 디테일 없음'); return; }
  console.log('\n=== [현대] ' + c.name + ' (ID:' + c.id + ') ===');
  console.log('트림 수:', d.trims ? d.trims.length : 0);
  if (d.trims) d.trims.forEach(t => console.log('  - ' + t.id + ': ' + t.name + ' = ' + (t.price/10000).toFixed(0) + '만원'));
